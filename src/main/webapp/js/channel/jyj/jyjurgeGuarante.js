...
$(document).ready(function(){
	...
	$.ajax({
		type : 'post',
		url : ctx + '/channel/jyj/urgeGuaranteeMoney/submitDeducts',
		data : {
		...
		success : function(result) {
				if(result=="success"){
					...
					window.location.href = ctx + '/channel/jyj/urgeGuaranteeMoney/urgeGuarantInfo';
				}else if(result=="deal"){
...
function urgeGuarantDeal(sid){
	window.location.href=ctx+'/channel/jyj/urgeGuaranteeMoney/urgeGuarantDeal?sid='+sid;
}
//格式化，保留两个小数点
function fmoney(s, n) {  
    n = n > 0 && n <= 20 ? n : 2;  
