		...
		art.dialog.confirm("是否确定批量匹配？", function(){
			...
			$.ajax({  
				   ...
				   data:{
					...
					url : ctx+"/borrow/grant/urgeCheckMatch/matchingPayback?" + urgeMatchingDate,
					datatype : "json",
					success : function(msg){
						window.setTimeout(function () { 
							top.$.jBox.tip(msg, 'success'); 
							window.location.href=ctx+"/borrow/grant/urgeCheckMatch/urgeCheckMatchInfo";
						}, 2000);
					},
					error: function(){
							top.$.jBox.closeTip()
		...
		art.dialog.confirm("是否确定批量退回？", function(){
			...
			$.ajax({  
				   ...
				   data:urgeServicesMoneyEx,
					url : ctx+"/borrow/grant/urgeCheckMatch/urgeMatchingBatchBack",
					datatype : "json",
					success : function(msg){
						top.$.jBox.tip(msg);
						window.location.href=ctx+"/borrow/grant/urgeCheckMatch/urgeCheckMatchInfo";
...
function page(n, s) {
	...
	if (s)
		$("#pageSize").val(s);
	$("#urgeMatchingForm").attr("action",  ctx + "/borrow/grant/urgeCheckMatch/urgeCheckMatchInfo");
	$("#urgeMatchingForm").submit();
	return false;
}