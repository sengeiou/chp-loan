...
$(document).ready(function(){
	...
	$.ajax({
		type : 'post',
		url : ctx + '/borrow/grant/urgeGuaranteeMoney/submitDeducts',
		data : {
			'rPaybackId':rPaybackId
		},
		success : function(result) {
				if(result=="success"){
					art.dialog.alert("提交划扣成功");
					window.location.href = ctx + '/borrow/grant/urgeGuaranteeMoney/urgeGuarantInfo';
				}else if(result=="deal"){
	...
	$("#urgeSaveApply").click(function(){
		...
		if(urgeApplyAmountSum < waitUrgeMoney){
			top.$.jBox.tip('实际到账金额不能小于待收取金额!');
			return false;
		}
...
function urgeGuarantDeal(sid){
	window.location.href=ctx+'/borrow/grant/urgeGuaranteeMoney/urgeGuarantDeal?sid='+sid;
}
//格式化，保留两个小数点
function fmoney(s, n) {  
    n = n > 0 && n <= 20 ? n : 2;  
