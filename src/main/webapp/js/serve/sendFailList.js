$(function(){
	...
	$("#btn-submit").click(function(){	
		$("#waitSendForm").attr("action",ctx+"/borrow/serve/emailServe/sendFailList");
		$("#waitSendForm").submit();
	});
 ...
 */
function send(sendEmailStatus,type,id,idType,emailType){
	var ids = '';
	var num=0;
	if(id!=''){
		ids=id;
	...
	var oldEmailType = $("#emailType").val();
	var msg=""
	...
	var dataArray = $("#waitSendForm").serialize();
	if(dataArray.indexOf("emailType")<0){
		dataArray = dataArray+"&emailType="+emailType;
	}
