 ...
 */
 function validateAllMobile(str){
	  ...
	  if(length == 11 && /^((\+?86)|(\(\+86\)))?(13[0123456789][0-9]{8}|15[012356789][0-9]{8}|17[678][0-9]{8}|18[02356789][0-9]{8}|147[0-9]{8}|1349[0-9]{7})$/.test(str)){
		  return "true";
	  }
	return  "请正确填写11位手机号";
}
...
function getValidErorTop(t){
	...
	t.find("input[type='text'],select,textarea").each(function(){
   	 if($(this).prop("class").toLowerCase().contains("required")){
   		  if ("" == $(this).val().trim()) { //判断元素对象的value值  
...
function getValidErorTopNei(t){
	 if(t.prop("class").contains("card")){
	   		if(isValidationIdCard(t.val())!='true'){
	   			 return t.position().top;
	   		}
	   	 }else if(t.prop("class").toLowerCase().contains("mobile")){
	   		 
	   		if(validateAllMobile(t.val())!='true'){
	   			return t.position().top;
	   		}
	   	 }else if(t.prop("class").toLowerCase().contains("simplephone")){
	   		 
	   	   		if(validateAllTel(t.val())!='true'){
	   	   		return t.position().top;
	   	   		}
	   	 }else if(t.prop("class").toLowerCase().contains("phone")){
	   		 
	   	   		if(validateAllPhone(t.val())!='true'){
	   	   		return t.position().top;
	   	   		}
	   	 }else if(t.prop("class").toLowerCase().contains("number")){
	   		 
	   		if(validateNumber(t.val())!='true'){
	   			return t.position().top;
	   		}
	   	 }else if(t.prop("class").toLowerCase().contains("integer")){
