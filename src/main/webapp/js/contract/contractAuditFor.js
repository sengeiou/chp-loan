	...
	function fetchTaskItems(srcForm,flagForm) {//queue='HJ_CONTRACT_COMMISSIONER' ，viewName = _rateAudit_workItems
		...
		window.location=ctx+'/contractAuditModule/searchContractAuditDatas?'+srcFormParam;
	}
	/**
	 全选事件
...
function workItemBackFlow(viewName,flowName,stepName,dealType,applyId,token,flowFlag,redirectUrl,wobNum){
	...
	function retAuditList(flowFlag){
		window.location=ctx+"/contractAuditModule/searchContractAuditDatas?flowFlag="+flowFlag;
		
	}
	function retAuditListNew(flowFlag,menuId){
		window.location=ctx+"/contractAuditModule/searchContractAuditDatas?flowFlag="+flowFlag+"&menuId="+menuId;
    		...
    		success : function(result) {
    			if(result.message=='手动验证通过'){
    				...
    				/*art.dialog.alert(result.message,function(){
        				window.location.href=ctx+"/contractAuditModule/searchContractAuditDatas?flowFlag=CTR_AUDIT";
        		  	});*/
    				window.location.href=ctx+"/contractAuditModule/searchContractAuditDatas?flowFlag=CTR_AUDIT";
    			}
    		},
