	 ...
	 function page(n, s) {
	   var menuId = $("#menuId").val();
	 	...
	 	if (s)
	 		$("#pageSize").val(s);
	 	$("#auditForm").attr("action",  ctx + "/borrow/payback/centraliDeduc/allCentralizedDeductionList?menuId="+menuId);
	 	$("#auditForm").submit();
	 	return false;
	 }
	...
	$(document).ready(function() {
		...
		$("button[name='exportExcel']").click(function() {
			...
			$("#ids").val(idVal);
			$("#auditForm").attr("action",  ctx + "/borrow/payback/centraliDeduc/exportExcel");
		 	$("#auditForm").submit();
		});
		...
		$("#clearBtn").click(function(){
			...
			$("#hdStore").val('');
			$("#bankId").val('');
			$("select[name = 'loanMark']").val('');
			$(":input").not(':button,:submit,:reset,:hidden').val('');
			$("#auditForm").attr("action",  ctx + "/borrow/payback/centraliDeduc/allCentralizedDeductionList");
		 	$("#auditForm").submit();
		});
		// 搜索按钮
		...
		$("#checkAll").click(function(){
			...
			if(this.checked){
				$("[name = 'checkBox']").each(function(){
					...
					totalreallyMoney = totalreallyMoney + parseFloat($(this).next().attr("reallyAmount") ? $(this).next().attr("reallyAmount"):0);
				})
				$("#totalNum").val(totalNum);
				$("#totalMoney").val(totalMoney);
				$("#total_reallyMoney").text(totalreallyMoney);
			}else{
				$("#totalNum").val(0);
				$("#totalMoney").val(0);
