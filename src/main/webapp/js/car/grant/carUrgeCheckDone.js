$(document).ready(function(){
	...
	$('#daoBtn').bind('click', function() {
		...
		window.location.href=ctx+"/car/grant/urgeCheckDone/urgeCheckDoneExl?checkApplyId="+checkApplyId+"&"+urgeCheck;
	});
...
function page(n, s) {
	...
	if (s)
		$("#pageSize").val(s);
	$("#urgeCheckDoneForm").attr("action",  ctx + "/car/grant/urgeCheckDone/urgeCheckDoneInfo");
	$("#urgeCheckDoneForm").submit();
	return false;
}
 ...
 * @param rUrgeId
 */
function showCarUrgeOpe(rUrgeId) {
	var url = ctx + "/car/grant/carUrgeHistory/showCarUrgeHistory?rUrgeId=" + rUrgeId;
	art.dialog.open(url, {
		id : 'his',
		title : '催收服务费操作历史',
		lock : true,
