...
<script type="text/javascript">
  ...
  function page(n, s) {
		...
		if (s)
			$("#pageSize").val(s);
		$("#inputForm").attr("action", "${ctx}/borrow/borrowlist/fetchTaskTelesales");
		$("#inputForm").submit();
		return false;
}
...
<body>
    ...
    <form:form id="inputForm" modelAttribute="loanFlowQueryParam" action="${ctx}/borrow/borrowlist/fetchTaskTelesales" method="post" class="form-horizontal">
              ...
              <c:when test="${item.backFlag=='1'}">
            	  <tr style="color:red;">
            		...
            		<td>${item.modelLabel}</td>
             		...
             		<td>
               		 <input type="button" value="办理" class="btn_edit"  onclick="window.location.href='${ctx}/bpm/flowController/openForm?applyId=${item.applyId}&wobNum=${item.wobNum}&dealType=0&token=${item.token}&loanInfoOldOrNewFlag=${item.loanInfoOldOrNewFlag}&teleFlag=${teleFlag}'"></input>
