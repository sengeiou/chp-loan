...
<script type="text/javascript">
		...
		$.ajax({
			...
			success : function(data){
				if (data != null){
					...
					if (data.newCustomerPhone != null && data.newCustomerPhone != ''){
						$("#newCustomerPhone").val(data.newCustomerPhone);
						$("#phone").val("0");
					} else {
						$("#newCustomerPhone").val(data.customerPhone);
						$("#phone").val("1");
					}
	...
	function lookGoldMessage(id){
		var url = "${ctx}/borrow/account/repayaccount/goldShowMessage?id=" + id;
		art.dialog.open(url, {  
	        id: 'look',
	        title: '查看',
	        lock:true,
...
<body>
	<div class="control-group">
		<form id="accountManageForm" action="${ctx}/borrow/account/repayaccount/accountManageList" method="post">
...
<%-- 						<input type="text" class="input-medium" id="repayDay" name="repayDay" value="${repayAccountApplyView.repayDay}"/> --%>
				...
				<c:forEach items="${rasList}" var="item">
					<tr>
						...
						<td>
							...
							<c:if test="${item.flag == 'TG'}">
								<button class="btn_edit" onclick="lookGoldMessage('${item.id}')">查看</button>
							</c:if>
	...
	<div id="examineDiv" class=" pb5 pt10 pr30 title ohide" style="display:none">
		<form id="examineMessage" class="validate" action="${ctx}/borrow/account/repayaccount/saveExamineResult" method="post" enctype="multipart/form-data">
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<input type="text" class="input-medium" id="newCustomerPhone" name="customerPhone" readonly="readonly"/>
						</td>
					</tr>
					<tr>
						<td>
					...
					<tr>
						...
						<!-- <td>
							...
							<a class="input-medium" id="phoneFile"></a>
						</td> -->
					</tr>
					<tr>
