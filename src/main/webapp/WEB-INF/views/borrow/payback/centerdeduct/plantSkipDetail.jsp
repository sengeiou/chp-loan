...
<div>
	...
	<form id="addOrEdit"  action="${ctx}/borrow/payback/plantskipordernew/save" method="post" class="form-horizontal">
		     <div id="onLineTable" class='table1'>
				<input type="hidden" class="cf_input_text178" name="id" value="${record.id}">
			  <div class="control-group" class="control-group">
						<label class='lab'>银行：</label>
						<select name="bankCode" required  class="select78"  style ="width:135px;" onChange = 'checkPlatform()'>
							<option value="">请选择</option>
	                		<c:forEach items="${fns:getDictList('jk_open_bank')}" var="d">
	                			<option value="${d.value}" 
	                				<c:if test="${d.value eq record.bankCode}">
	                					selected
	                				</c:if> 
	                			>${d.label}</option>
	                		</c:forEach>
               ...
               <c:choose>
            	<c:when test="${get eq 'get' }">
            		     <c:forEach items="${record.platDeductTypeList}" var="platformIdList" varStatus="status">
            				<c:if test="${status.index == 0 }">
            				       <div class="control-group">
										<label class='lab'>第一平台：</label>
										<select name="platIdList" required  class="select78"  style ="width:135px;" disble>
											<option value="">请选择</option>
					                		<c:forEach items="${fns:getDictList('jk_deduct_plat')}" var="d">
					                		   <c:if test="${d.value != '4'}">
					                			<option value="${d.value}" 
					                				<c:if test="${d.value eq platformIdList.name}">
					                					selected
					                				</c:if> 
					                			>${d.label}</option>
					                			</c:if>
					                	...
					                	<c:if test="${platformIdList.name != '3' }">
										<label class='lab'>接口：</label>
							            <select  required  class="select78"  name ="deductTypeList"  style ="width:135px;">
													<option value="">请选择</option>
							                		<c:forEach items="${fns:getDictList('com_deduct_type')}" var="d">
							                			<option value="${d.value}" 
							                				<c:if test="${d.value eq platformIdList.code}">
							                					selected
							                				</c:if> >${d.label}
							                			</option>
							                		</c:forEach>
							             ...
							             <c:if test="${platformIdList.name == '3' }">
										<label class='lab'  style="display:none">接口：</label>
							            <select  required   style="display:none" class="select78"  name ="deductTypeList"  style ="width:135px;">
													<option value="">请选择</option>
							                		<c:forEach items="${fns:getDictList('com_deduct_type')}" var="d">
							                			<option value="${d.value}" 
							                				<c:if test="${d.value eq platformIdList.code}">
            				...
            				<c:if test="${status.index != 0 }">
            					<div class="control-group">
										...
										<select name="platIdList" class="select78">
											<option value="">请选择</option>
					                		<c:forEach items="${fns:getDictList('jk_deduct_plat')}" var="d">
					                			   <c:if test="${d.value != '4'}">
					                			<option value="${d.value}" 
            	...
            	<c:otherwise>
            		<div class="control-group" class="control-group">
							<label class='lab'>第一平台：</label>
							<select name="platIdList" required  class="select78"  style ="width:135px;" onchange='checkPlatform()'>
								<option value="">请选择</option>
		                		<c:forEach items="${fns:getDictList('jk_deduct_plat')}" var="d">
		                            <c:if test="${d.value != '4'}">
		                			<option value="${d.value}" 
		                	...
		                	<label class='lab'>接口：</label>
		                	<select name="deductTypeList" required  class="select78"  style ="width:135px;">
								<option value="">请选择</option>
		                		<c:forEach items="${fns:getDictList('com_deduct_type')}" var="d">
		                			<option value="${d.value}" 
		                				<c:if test="${d.value eq deductType}">
		                					selected
		                				</c:if> 
		                			>${d.label}</option>
		                		</c:forEach>
                 ...
                 <div class="control-group">
					...
					<label>启用</label>
					<input type="radio" required  name="status" <c:if test="${record.status eq '1'}">checked</c:if> value="1">
					<label>停用</label>
					<input type="radio" required  name="status" <c:if test="${record.status eq '0'}">checked</c:if>  value="0">
				</div>
                <div class="control-group">
					<label class='lab'>是否集中：</label>
					<label>是&nbsp;&nbsp;&nbsp;</label>
					<input type="radio" required  name="isConcentrate" onclick = 'checkPlatform()' <c:if test="${record.isConcentrate eq '1'}">checked</c:if>  value="1">
					<label>否&nbsp;&nbsp;&nbsp;&nbsp;</label>
					<input type="radio" required  name="isConcentrate" onclick = 'checkPlatform()'  <c:if test="${record.isConcentrate eq '0'}">checked</c:if> value="0">
				</div>
            <!--隐藏的tr,功能实现的需要  -->
            <div id="template" style="display:none" class="control-group">
					<label class='lab'>自动跳转平台：</label>
					<select class="select78" id="platIdList" style ="width:135px;">
						<option value="">请选择</option>
                	...
                	<label class='lab'>接口：</label>
		             <select  required  class="select78"  id ="deductTypeList"  style ="width:135px;">
								<option value="">请选择</option>
		                		<c:forEach items="${fns:getDictList('com_deduct_type')}" var="d">
		                			<option value="${d.value}" 
		                				<c:if test="${d.value eq deductType}">
