...
<script type="text/javascript" src="${context }/js/payback/applyPayback.js"></script>
...
<body>
	...
	<form id="applyPayLaunchForm" enctype="multipart/form-data" action="${ctx}/borrow/payback/applyPayback/saveApplyPayLaunch" method="post">
		...
		<input type="hidden" id="confrimFlag" name="paybackApply.confrimFlag" value="0">
		<input type="hidden" id="sumDayLimit" name="sumDayLimit" value="0">
		<input type="hidden" id="zjcnt" name="zjcnt" value="0">
		<input type="hidden" id="applyTokenId" name="applyTokenId"  />
		<input type="hidden" id="applyToken" name="applyToken"/>
		<input type="hidden" id="channelFlag" name="contract.channelFlag" value="${payback.contract.channelFlag }"/>
			...
			<table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
				<tr>
					<td><label class="lab">合同编号：</label> 
						...
						<input type="hidden" id="isfive" value="${isfive }">
						<input type="hidden" id="checkDay" value="${checkDay }">
					</td>
			...
			<table cellpadding="0" cellspacing="0" border="0" width="100%">
				...
				<tr>
					<td ><label class="lab"> 还款方式：</label> 
						...
						<div id="deductDiv" style="display:inline"><input type="radio" style="margin-top:5px" id="paymentsAuto" name="paybackApply.dictRepayMethod" value="0">划扣</div>
						<input type="radio" style="margin-top:5px" id="paymentsPosCard" name="paybackApply.dictRepayMethod" value="2">POS机刷卡
						<input type="radio" style="margin-top:5px" id="paymentsPosAudit" name="paybackApply.dictRepayMethod" value="3">POS机刷卡查账
					</td>
					<td> 
						<input type="checkbox" style="margin-top:5px" id="operateRole" name="paybackApply.operateRole" value="1">中和公司查账
					</td>
				</tr>
			</table>
		</div>
		...
		<div id="qishu_div0" style="display: none;">
			<table id='appendTab' class="table table-bordered table-condensed">
				...
				<tr id='appendId'>
					<td><input type="text" class="input_text100" width="128px" id="dictDeposit" name="paybackTransferInfo.dictDeposit" /></td>
					<td><input type="text" class="input_text100" id="accountBranch" name="paybackTransferInfo.accountBranch" maxlength="20" /></td>
					...
					<td>
						<input type="text" id="applyTime" name="paybackTransferInfo.applyTimeStr" maxlength="20" class="input-medium Wdate" style="cursor: pointer"
						 value="<fmt:formatDate value="${paybackApply.applyTime}" pattern="HH:mm"/>" 
						 onclick="WdatePicker({dateFmt:'HH:mm'});"/>
					</td>
					<td><input type="text" class="input_text70" id="applyAmountCopy" name="paybackTransferInfo.reallyAmountStr" /></td>
					<td><input type="text" class="input_text70" id="depositName" name="paybackTransferInfo.depositName" /></td>
					<td><input name="files" type="file"></td>
					<td><input type="text" class="input_text70" id="uploadName" name="paybackTransferInfo.uploadName" / readonly />&nbsp;</td>
					<td><input type="text" class="input_text70" id="uploadDate" name="paybackTransferInfo.uploadDate" readOnly /></td>
				</tr>
			</table>
		</div>
					...
					<td><label class="lab">划扣平台：</label> 
						<select class="select178" id="dictDealType" name="paybackApply.dictDealType" required>
							...
							<c:forEach items="${fns:getDictList('jk_deduct_plat')}" var="item">
								...
								<option value="${item.value}">${item.label}</c:if>
							</c:forEach>
						</select>
					</td>
					<td><label class="lab">还款申请日期：</label> 
