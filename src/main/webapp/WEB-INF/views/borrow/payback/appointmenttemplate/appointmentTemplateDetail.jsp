...
<title><c:if test="${ bean.id == '' || bean.id == null }">新增预约模板</c:if>
        <c:if test="${ bean.id != '' && bean.id != null }">修改预约模板</c:if>
</title>
...
$(function(){
	$("#resets").click(function(){
		var url = ctx+"/borrow/payback/appointmenttemplate/queryList";
...
function formSubmit(){
  ...
  if(!loanStatusId){
	...
	var url = ctx+"/borrow/payback/appointmenttemplate/insertOrUpdate";
	var flag = $("#beanId").val();
	var msg = "";
	if(flag){
		msg ="修改成功！"
               ...
               <td><label class="lab" >银行：</label>
                ...
                <td><label class="lab">预约时间：</label>
				  <input name="appointmentDay" id="appointmentDay"  type="text" readonly="readonly"  maxlength="40" class="input-medium Wdate"
				    value="<fmt:formatDate value="${bean.appointmentDay}" pattern="HH:mm"/>"
                ...
                <td><label class="lab">规则：</label>
                      <select class="select180" name="ruleCode">
                         <c:forEach var="rule" items="${mapRule}">
								...
								<c:if test="${bean.ruleCode==rule.key }">selected</c:if>>${rule.value }</option>
					    </c:forEach>
                      </select>
                </td>
