...
function page(n,s) {
	if (n) $("#pageNo").val(n);
	if (s) $("#pageSize").val(s);
	$("#auditForm").attr("action", "${ctx}/borrow/payback/appointmenttemplate/queryList");
	$("#auditForm").submit();
	return false;
}
$(document).ready(function() {
 ...
 $("#cleBtn").click(function(){
  ...
  $("#newAppoint").click(function(){
		var url = ctx+"/borrow/payback/appointmenttemplate/goDetail";
  ...
  $("#editAppoint").click(function(){
	   var obj = $("input:checkbox[name='checkBox']:checked");
		...
		var  id = obj.val();
		var url = ctx+"/borrow/payback/appointmenttemplate/goDetail?id="+id;
		window.location.href = url;
  })
   ...
   $("#deleteAppoint").click(function(){
			    ...
			    /*  var status = obj.attr("status");
				 if(status == '0'){
			    	 ...
			    	 art.dialog.confirm("请确认是否删除此条数据!", function () {
				       var url = ctx+"/borrow/payback/appointmenttemplate/deleteData";
				       var id = obj.val();
				               // 单条修改
						    	$.ajax({
						        type: "POST",
	...
	$("#realdeductForm").bind('click', function() {
      ...
      $("#exportForm").click(function(){
    	 art.dialog.confirm('确认导出？', 
				    function () {
		    		  var idVal ='';
		        	  $("input:checkbox[name='checkBox']:checked").each(function(){
		      			 idVal+=","+$(this).val();
		      		  });
		      		  url=ctx+"/borrow/payback/appointmenttemplate/exportExcel"; 
	 ...
	 if(length == 1){
		 ...
		 if(sign ==status ){
			 ...
			 if(sign == '1'){
			  ...
			  var url = ctx+"/borrow/payback/appointmenttemplate/changSingleEffect";
			  var id = obj.val();
			  // 单条修改
				$.ajax({
			        type: "POST",
		 ...
		 art.dialog.confirm(msg, function () {
			 if(length > 1){
			   ...
			   $("#signStatus").val(sign);
                var url = ctx+"/borrow/payback/appointmenttemplate/changEffect";
				$.ajax({
			        type: "POST",
			        url: url,
			        async: false,
			        ...
			        success: function(data){
			        	 if(data.success == true){
			        			art.dialog.alert(data.msg,function(){
			        				 window.location.href = ctx+"/borrow/payback/appointmenttemplate/queryList";
			               		});
			        	  }else{
			        		  art.dialog.alert(data.msg);
			        	  }
...
function ajaxFileUpload() {
     ...
     var url = ctx+"/borrow/payback/appointmenttemplate/importRule"; 
...
<div class="control-group">
    <form id="auditForm" method="post" action="${ctx}/borrow/payback/appointmenttemplate/queryList" >
                ...
                <td><label class="lab">银行：</label>
				 ...
				 <td><label class="lab">预约时间：</label>
				  <input name="appointmentDay" id="appointmentDay"  type="text" readonly="readonly" maxlength="40" class="input-medium Wdate"
					    value="<fmt:formatDate value="${paramMap.appointmentDay}" pattern="HH:mm"/>"
					     onclick="WdatePicker({dateFmt:'HH:mm'});" style="cursor: pointer"/>
			    </td>
            </tr>
  ...
  <p class="mb5">
   <button class="btn btn-small" id="newAppoint">新增预约模板</button>
   <button class="btn btn-small" id="deleteAppoint">删除</button>
   <button class="btn btn-small" id="editAppoint">修改</button>
   <button class="btn btn-small" id="realdeductForm" >导入规则</button>
   <button class="btn btn-small" id="exportForm" >导出Excel</button>
            ...
            <tr>
                 ...
                 <td>
                  ...
                  <c:if test="${empty bean.applyReallyAmount}">
                   ...
                   <fmt:formatNumber value="${bean.applyReallyAmount}" pattern='0.00'/>
                 </td>
                 <td>
