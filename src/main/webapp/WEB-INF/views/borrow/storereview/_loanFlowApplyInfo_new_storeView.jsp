...
<body>
 <ul class="nav nav-tabs">
	...
	<li class="active"><a href="javascript:launch.getCustomerInfo_storeView('2');">借款意愿</a></li>
...
<form id="applyInfoForm" method="post">
	 ...
	 <input type="hidden" name="loanCustomer.id" value="${workItem.bv.loanCustomer.id}"/>
	 <input type="hidden" name="loanCustomer.loanCode" value="${workItem.bv.loanCustomer.loanCode}"/>
     <input type="hidden" name="loanInfo.id" value="${applyInfoEx.loanInfo.id}"/>
	 <input type="hidden" name="loanInfo.loanCode" value="${applyInfoEx.loanInfo.loanCode}"/>
 	 ...
 	 <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
		    <td>
		   	...
		   	<td>
		   		...
		   		<!-- 退回门店 和 复议退回门店 不允许修改产品类别 -->
				<select id="productId" name="loanInfo.productType" value="${applyInfoEx.loanInfo.productType}" class="select180 required" <c:if test="${'21' eq workItem.bv.lastLoanStatus || '41' eq workItem.bv.lastLoanStatus}">disabled="disabled"</c:if>>
		...
		<tr>
			...
			<td>
				...
				<input type="text" name="loanInfo.otherCompanyPaybackCount" id="otherCompanyPaybackCount" value="${applyInfoEx.loanInfo.otherCompanyPaybackCount}" class="input_text178 digits required" maxlength="3"/> 
			</td> 
			<td>
...
$(document).ready(
	function() {
		$('#applyInfoNextBtn').bind('click',function(){
			...
			launch.updateApplyInfo('2','applyInfoForm','applyInfoNextBtn');
		});
