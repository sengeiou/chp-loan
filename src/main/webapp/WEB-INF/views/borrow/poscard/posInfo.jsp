...
<body>
	<form id="doStoreForm" action="" method="post" class="form-horizontal">
			...
			<table class=" table1" cellpadding="0" cellspacing="0" border="0"
				...
				<tr>
					...
					<td><label class="lab">借款状态：</label> 
						<input type="text" class="input_text178" id="dictLoanStatus" name="loanInfo.dictLoanStatus" value="${paybackApply.loanInfo.dictLoanStatus }" readonly>
					</td>
				</tr>
			</table>
		</div>
		...
		<div id="qishu_div3">
			...
			<table class="" cellpadding="0" cellspacing="0" border="0" width="100%">
				...
				<input type="radio" id="paymentsPosAudit" name="dictRepayMethod" value="3"<c:if test="${paybackApply.dictRepayMethod=='3'}"> checked</c:if> disabled/>POS机刷卡查账
					</td>
				</tr>
			</table>
		</div>
					...
					<td><label class="lab">存入账户：</label> 
						<select class="select78_24" id="storesInAccount" name="paybackTransferInfo.storesInAccount">
							<c:forEach items="${fns:getDictList('jk_open_bank')}" var="applyBankName">
                                  <option value="${applyBankName.value }" <c:if test="${paybackApply.paybackTransferInfo.storesInAccount==applyBankName.value }">selected</c:if>>${applyBankName.label }</option>
                            </c:forEach>
						</select>
					</td>
					<td><label class="lab">还款申请日期：</label>&nbsp; 
				...
				<c:forEach items="${paybackTransferInfoList}" var="item">
					<tr id='appendId'>
						...
						<td><input name="files" type="file">
							<input type="button" class="btn_edit"  name = "previewPngBtn" value="${item.uploadFilename }" docId = ${item.uploadPath }></td>
