...
$(document).ready(function(){
	...
	if("${applyInfoEx.loanCompManage.managePlace}".indexOf("1") != -1){
		...
		"<input type='text' style='width: 100px;' class='required number numCheck positiveNumCheck' min='0' max='999999999.00' id='monthRentMoney' pattern='0.00' name='loanCompManage.monthRentMoney' value='${applyInfoEx.loanCompManage.monthRentMoney}'></span>");
	}
	if("${applyInfoEx.loanCompManage.managePlace}".indexOf("2") != -1){
		...
		"<input type='text' style='width: 100px;' class='required number numCheck positiveNumCheck' min='0' max='999999999.00' id='monthPayMoney' pattern='0.00' name='loanCompManage.monthPayMoney' value='${applyInfoEx.loanCompManage.monthPayMoney}'></span>");
	}
	
	$('#manageNextBtn').bind('click',function(){
		$(this).attr('disabled',true);
	 	launch.saveApplyInfo('7','manageForm','manageNextBtn'); 
	});
   ...
   function orgCodeBlur(){
	...
	function managePlaceHandler(mark){	
		...
		if(mark.value == '1' && mark.checked == true){
			$("#possession").append("<span id='appendRent'><label id='monthRentMoneyTitle' class='lab'><span class='red'>*</span>月租金(元)：</label>"+
					"<input type='text' id='monthRentMoney' class='required number numCheck positiveNumCheck' min='0' max='999999999.00' pattern='0.00' name='loanCompManage.monthRentMoney' value='${applyInfoEx.loanCompManage.monthRentMoney}'></span>");
		...
		//选中按揭显示月还款
			...
			$("#possession").append("<span id='appendPay'><label id='monthPayMoneyTitle' class='lab'><span class='red'>*</span>月还款(元)：</label>"+
			"<input type='text' id='monthPayMoney' class='required number numCheck positiveNumCheck' min='0' max='999999999.00' pattern='0.00' name='loanCompManage.monthPayMoney' value='${applyInfoEx.loanCompManage.monthPayMoney}'></span>");
		}
...
<ul class="nav nav-tabs">
		...
		<li class="active"><a href="javascript:launch.getCustomerInfo('7');">经营信息</a></li>
	...
	<div id="div7" class="content control-group">
		<form id="manageForm" method="post">
			...
			<input type="hidden" value="${workItem.flowType}" name="flowType"></input>
		   <input type="hidden" value="${workItem.bv.defTokenId}" name="defTokenId"/>
		   <input type="hidden" value="${applyInfoEx.loanInfo.loanApplyAmount}" ></input>
			      ...
			      <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
					...
					<tr>
						<td>
							...
							<input type="text" name="loanCompManage.averageMonthTurnover" pattern="0.00" class="input_text178 number numCheck positiveNumCheck" min="0" max="999999999.00" value="${applyInfoEx.loanCompManage.averageMonthTurnover}"/>
						</td>
						<td>
					...
					<tr>
						<td>
							...
							<input type="text" name="loanCompManage.customerRatioComp" pattern="0.00" class="input_text178 number numCheck positiveNumCheck" min="0" max="999999999.00" value="${applyInfoEx.loanCompManage.customerRatioComp}"/>
						</td>
						<td>
		...
		<div class="tright mr10 mb5" >
			<input type="submit" id="manageNextBtn" class="btn btn-primary" value="下一步" />
		</div>
		</form>
	</div>
</body>
