...
$(document).ready(function(){
	$('#coborrowNextBtn').bind('click',function(){
	     ...
	     coborrower.format();
		 launch.saveApplyInfo('5','coborrowForm','coborrowNextBtn');
	}); 
	...
	//自然人保证人初始化
	coborrower.naturalGuarantorInit("apply");
});
var msg = "${message}";
if (msg != "" && msg != null) {
	top.$.jBox.tip(msg);
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="javascript:launch.getCustomerInfo('5');">自然人保证人信息</a></li>
	...
	<div id="div4" class="content control-group ">
		...
		<form id="coborrowForm" method="post">
			...
			<!-- js校验时，区分申请和代办 -->
			<input type="hidden" id = "page" value="apply">
								...
								<tbody>
											...
											<tr>
												...
												<td class="tcenter">
													<c:if test="${relatives.index>0}">
														<input type="button" class="btn_edit" value="删除" onclick="contact.delRow(this,'relatives_contact_table_${coboStatus.index}','CONTACT')" style="width: 50px;" />
													</c:if>
												</td>
											</tr>
										</c:forEach>
									...
									<c:if test="${empty currCoborro.relativesContactList }">
										<tr>
											...
											<td>
												<input id="relativesContact_${coboStatus.index}_contactMobile_0" type="text" name="loanCoborrower[${coboStatus.index}].coborrowerContactList.contactMobile" class="input_text178 isMobile contactMobileDiff1 mobileAndTelNeedOne" />
											</td>
											<td class="tcenter"></td>
										</tr>
									</c:if>
											...
											<tr>
												...
												<td class="tcenter">
													<c:if test="${workTogether.index>0}">
														<input type="button" class="btn_edit" value="删除" onclick="contact.delRow(this,'worktogether_contact_table_${coboStatus.index}','CONTACT')" style="width: 50px;" />
													</c:if>
												</td>
											</tr>
										</c:forEach>
											...
											<tr>
												...
												<td class="tcenter">
													...
													<c:if test="${other.index>otherContactDelRow}">
														<input type="button" class="btn_edit" value="删除" onclick="contact.delRow(this,'other_contact_table_${coboStatus.index}','CONTACT')" style="width: 50px;" />
													</c:if>
												</td>
											</tr>
										</c:forEach>
