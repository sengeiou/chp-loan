 	...
 	$(document).ready(function(){
		$('#contactNextBtn').bind('click',function(){
			contactFormat.format();
	   	 	...
	   	 	launch.saveApplyInfo('4','contactForm','contactNextBtn');
	 	});
		//初始化
		contact.init();
		//判断是否选择其他
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="javascript:launch.getCustomerInfo('4');">联系人信息</a></li>
	...
	<div id="div8" class="content control-group">
		<form id="contactForm" method="post">
					...
					<td colspan="2">
						...
						<select class="select78 mr10 required" id="mateAddressProvince" name="loanMate.mateAddressProvince" value="${applyInfoEx.loanMate.mateAddressProvince}">
							...
							<c:forEach var="item" items="${provinceList}" varStatus="status">
								<option value="${item.areaCode}" <c:if test="${applyInfoEx.loanMate.mateAddressProvince==item.areaCode}">selected=true</c:if>>${item.areaName}</option>
							</c:forEach>
						</select>
						-
					...
					<c:forEach items="${applyInfoEx.relationContactList}" var="ccItem" varStatus="ccStatus">
						<tr>
							<td>
								...
								<input id="relaticontact_contactName_${ccStatus.index}" type="text" name="customerContactList.contactName" value="${ccItem.contactName}" class="input_text180 stringCheck required contactCheck" />
							</td>
							<td>
							...
							<td class="tcenter">
								<c:if test="${ccStatus.index>0}">
									<input type="button" onclick="contact.delRow(this,'table_contactRelationArea','CONTACT');" class="btn_edit" value="删除" />
								</c:if>
							</td>
						</tr>
					</c:forEach>
				...
				<c:if test="${applyInfoEx.workProveContactList != null && fn:length(applyInfoEx.workProveContactList) > 0}">
					<c:forEach items="${applyInfoEx.workProveContactList}" var="ccItem" varStatus="ccStatus">
						<tr>
							...
							<td class="tcenter">
								<c:if test="${ccStatus.index>0}">
									<input type="button" onclick="contact.delRow(this,'table_workProvePersonArea','CONTACT');" class="btn_edit" value="删除" />
								</c:if>
							</td>
						</tr>
					</c:forEach>
						...
						<tr>
							...
							<td class="tcenter">
								...
								<c:if test="${ccStatus.index>otherContactDelRow}">
									<input type="button" onclick="contact.delRow(this,'table_otherArea','CONTACT');" class="btn_edit" value="删除" />
								</c:if>
							</td>
						</tr>
					</c:forEach>
					...
					<td colspan="3">
						...
						<span>
							<c:forEach items="${fns:getNewDictList('jk_who_can_know_the_borrowing')}" var="item">
								<input type="checkbox" class="required" name="loanCustomer.whoCanKnowBorrow" id="knowContactPerson${item.value}" onclick="knowBorrowMoneyHandle(this);" <c:if test="${fn:contains(applyInfoEx.loanCustomer.whoCanKnowBorrow,item.value)}">checked</c:if> value="${item.value}">${item.label}
							</c:forEach>
						</span>
			...
			<div class="tright mb5 mr10">
				<input type="submit" id="contactNextBtn" class="btn btn-primary" value="下一步" />
			</div>
		</form>
	</div>
</body>
