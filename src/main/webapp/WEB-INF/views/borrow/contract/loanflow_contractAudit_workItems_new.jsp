 ...
 $(document).ready(function(){
    ...
    function dealTaskNewTwo(applyId,loanCode, dealType, menuId) {
	  ...
	  $(":input[name='backBtn']").each(function(){
		     $(this).bind('click',function(){
		    	 workItemBackFlow1('audit_grantBackDialog',$(this).attr('flowName'),$(this).attr('stepName'),$(this).attr('dealType'),$(this).attr('applyId'),$(this).attr('token'),$('#flowFlag').val(),REDIRECT_URL,$(this).attr('wobNum')); 
		     });
		 });
    	...
    	var split = false;
      	...
      	var contractCodes = "";
    		...
    		if(param == ""){
  	    	...
  	    	backListFlow('auditBackList',$('#flowFlag').val(),"/contractAuditModule/searchContractAuditDatas",$('#menuId').val(),param,fromSubmit);  //传递退回窗口的视图名称
        });
  });
							...
							<c:choose>
							  <c:when test="${item.loanStatusName=='待款项确认退回'||item.loanStatusName=='放款退回' || item.loanStatusName=='金信退回' || item.backFlag=='1' || item.loanStatusCode=='104' || item.loanStatusCode=='105'}">
							    <tr class="red">
							    	...
							    	<td> 
									  <!--    不拆分的数据 按钮显示   -->
										<c:if test="${item.isSplit ==issplit0 }">
										...
										<!--    拆分的数据 按钮显示   -->
										<c:if  test="${item.isSplit ==issplit1 }">
											<c:if test="${item.backReason!='风险客户' && item.backReason!='客户主动放弃' && item.frozenFlag!='1' && item.loanStatusCode!='104'}">
										    <input type="button" class="btn_edit"   applyId='${item.applyId}'  loanCode="${item.loanCode}"  dealType='0'  name="dealctrAuditNew" value="办理" />
											</c:if>
											
											<%-- <input type="button" class="btn_edit" applyId='${item.applyId}'   dealType='0'  name="backBtnNew" value="退回"/> --%>
										</c:if>
