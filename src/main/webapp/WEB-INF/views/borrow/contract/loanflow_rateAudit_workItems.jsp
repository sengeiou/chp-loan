...
<html>
<head><!--  -->
<script src="${context}/js/contract/contractAudit.js?version=6" type="text/javascript"></script>
...
<link rel="stylesheet" href="${context}/static/jquery-mloading/jquery.mloading.css">
<script src="${context}/static/jquery-mloading/jquery.mloading.js"></script>
<script src="${context}/js/contract/channelHandler.js?version=4" type="text/javascript"></script>
...
$(document).ready(
  function() {
						...
						function() {
		    				dealTaskNew($(this).attr('applyId'),
									$(this).attr('wobNum'), $(this)
											.attr('dealType'), $(
											this).attr('token'),$(this).attr("stepName"),'${param.menuId}');
	...
	/* 	// 添加无纸化标识
	    ...
	    $('#splitViewBtn').bind('click',function(){
	    	splitView();
	    });
	    $('#splitViewHisBtn').bind('click',function(){
	    	splitViewHis();
	    });
	    var msg = "${message}";
		if (msg != "" && msg != null) {
		 ...
		 $('#backListBtn').bind("click",function(){
		    	var param = "";
		  		...
		  		var fromSubmit = "";
		  		if(param == ""){
		  			fromSubmit = fetchTaskItemsSubmit('rateAuditForm','flagForm');
		  		}
...
function timeCheck(obj){
	var outApproveTimeEnd = $("#outApproveTimeEnd").val();
	var outApproveTimeStart= $("#outApproveTimeStart").val();
...
<body>
	<div>
    	<div class="control-group">
    	  ...
    	  <input type="hidden" id="batchCollBack"/>
    	  <input type="hidden" id="errorCountZCJ"/>
    	  <input type="hidden" id="sucessCountZCJ"/>
		  <form id="rateAuditForm">
		  <input type="hidden" name="menuId" id="menuId1" value="${param.menuId }">
				   ...
				   <td><label class="lab" >产品：</label>
					...
					<tr id="T1" style="display: none;" >
					 ...
					 <tr id="T3" style="display: none;" >
						 <td><label class="lab">汇诚审批时间：</label>
				               <input type="text" id="outApproveTimeStart"  name="outApproveTimeStart" readonly="readonly" value="${ctrQryParam.outApproveTimeStart}" onchange="timeCheck(this)" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate input_text180"/>-
				               <input type="text" id="outApproveTimeEnd" name="outApproveTimeEnd" readonly="readonly" value="${ctrQryParam.outApproveTimeEnd}" onchange="timeCheck(this)" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate input_text180"/>
              			 </td>
						 <td></td>
						 <td>
						 </td>
		...
		<div  id="auditList">
			...
			<p class="mb5">
				...
				<!-- <button class="btn btn-small" id="cancelTGBtn">批量取消TG</button>
				<button class="btn btn-small" id="addTGBtn">批量增加TG</button> -->
			<!--<button class="btn btn-small" id="addPaperLessBtn">添加无纸化标识</button>
				<button class="btn btn-small" id="cancelPaperLessBtn">取消无纸化标识</button> -->
				<c:if test="${isRateLeader=='1'}">
					<button class="btn btn-small jkhj_rateaudit_percent_view" id="percentViewBtn">占比查看</button>
				</c:if>
				<c:if test="${isRateLeader=='1'}">
				<button class="btn btn-small rate_backList" id="backListBtn">批量退回</button>
				</c:if>
				<!-- 占比分配暂时屏蔽 -->
				<c:if test="${isRateLeader=='1'}">
				<button class="btn btn-small jkhj_splitView" id="splitViewBtn">信息平台占比分配</button>
				</c:if>
				<button class="btn btn-small jkhj_splitViewHis" id="splitViewHisBtn">占比历史</button>
				...
				<tbody id="prepareListBody">
					<c:if test="${ workItems!=null && fn:length(workItems)>0}">
						...
						<c:forEach items="${workItems}" var="item">
							  ...
							  <c:when test="${item.backFlag=='1' || item.nodeFlag=='1'}">
							    <tr class="red">
								 ...
								 <input type="button" class="btn_edit"
									...
									token='${item.token}' stepName="${item.stepName}" months="${item.replyMoney}"  loanCode="${item.loanCode}" name="dealRateAudit" value="办理" />
							...
							<c:otherwise>
							   <tr>
								 <td><input type="checkbox" name="prepareCheckEle" backFlag="0"  paperLessFlag='${item.paperLessFlag}' flag='${item.channelName}' 
								  model="${item.model}" replyProductName='${item.replyProductName}' 
								  value='${item.applyId},${item.wobNum},${item.flowName},${item.token},${item.stepName},${item.loanCode}' />${index}</td>
								 <td>${item.contractCode}</td>
								 <td>
