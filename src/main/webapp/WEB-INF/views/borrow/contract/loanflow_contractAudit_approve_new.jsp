...
<script src="${context}/js/contract/contractAuditNew.js" type="text/javascript"></script>
...
var imgURL ="${bview.imageUrl}";
var largeAmountImgUrl = "${bview.largeAmountImageUrl}";
		    ...
		    button:[
		            {
		            	...
		            	callback:function(){
		        		    ...
		        		    dispatchFlowWithPermission(dispatchParam.flowFlag,dispatchParam.response,dispatchParam.redirectUrl,'${param.menuId}');
...
<div style="position:absolute;top:0;left:0;width:100%;height:95%">
  ...
  <input type="hidden" id="flagCancelParam" value="${bview.applyId},${bview.loanCode}"/>
    <form id="loanapplyForm">
      <input type="hidden" value="${bview.applyId}" name="applyId"></input>
      <input value="合同审核" name="stepName" type="hidden" />
    ...
    <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
	  ...
	  <tr>
        <td><label class="lab">合同编号：</label><label class="enlarge">${bview.contract.contractCode}</label>
         ...
         <input type="hidden" value="${bview.oldLoanCode}"  id="oldloanCode"></input>
         ...
         <input type="hidden" value="${bview.loanFlag}" name="contract.channelFlag" id="channelCode"/>
         <input type="hidden" value="${bview.loanFlagName}" id="channelName"/>
      ...
      <c:forEach items="${bview.coborrowers}" var="currItem">
           <tr>
			...
			<tr>
			    <td>
			    	<label class="lab">
			    		<c:if test="${bview.loanInfoOldOrNewFlag eq '' || bview.loanInfoOldOrNewFlag eq '0'}">
							共借人姓名：
						</c:if>
						<c:if test="${bview.loanInfoOldOrNewFlag eq '1'}">
							自然人保证人姓名：
						</c:if>
			    	</label>
			    	${currItem.coboName}
	  ...
	  <tr>
		...
		<td>
    		 <label class="lab">渠道：</label>
			   <span id="loanFlag">${bview.loanFlagName}</span>
       </td>
	  </tr>
	  <tr>
