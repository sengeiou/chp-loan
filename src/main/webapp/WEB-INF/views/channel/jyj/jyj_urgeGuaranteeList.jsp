...
<script language="javascript"> 
	function page(n, s) {
		...
		if (s)
			$("#pageSize").val(s);
		$("#backForm").attr("action", "${ctx}/channel/jyj/urgeGuaranteeMoney/urgeGuarantInfo");
		$("#backForm").submit();
		return false;
	}
</script>
...
<body>
   ...
   <form:form action="${ctx }/channel/jyj/urgeGuaranteeMoney/urgeGuarantInfo" method="post" id="backForm" modelAttribute="UrgeServicesMoneyEx">
        ...
        <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <tr>
				 <%-- <td><label class="lab">借款类型：</label><form:select class="select180" path="classType">
						<option>请选择</option>
						<c:forEach items="${fns:getDictList('jk_loan_type')}" var="card_type">
		...
		<p class="mb5">
    	&nbsp;&nbsp;<label class="lab1"><span class="red">待催收累计金额：</span></label>
        ...
        <tbody>
         ...
         <c:set var="index" value="0"/>
          ...
          <c:set var="index" value="${index+1}" />
             ...
             <td><fmt:formatNumber value='${item.allFee}' pattern="#,#00.00"/></td>
             <td><fmt:formatNumber value='${item.feeCount}' pattern="#,#00.00"/></td>
             ...
             <td><fmt:formatDate value="${item.lendingTime }" type="date" /></td>
