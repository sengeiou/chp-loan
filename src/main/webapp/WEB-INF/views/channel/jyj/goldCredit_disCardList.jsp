				...
				if($(":input[name='checkBoxItem1']:checked").length==0){
					art.dialog.alert('请选择要进行操作的数据!');
			           return;					
						...
						if (!frozenFlag)
							window.location.href=ctx +"/channel/jyj/JyjBorrowBankConfigure/disCardJump?checkVal="+checkVal;
						else 
				...
				success : function(data) {
					if(data!=null){
						art.dialog.alert('退回成功!',function (){
							window.location=ctx+'/channel/jyj/JyjBorrowBankConfigure/getGCDiscardInfo';
						});
					}else{
...
function page(n, s) {
	...
	if (s)
		$("#pageSize").val(s);
	$("#disCardForm").attr("action","${ctx }/channel/jyj/JyjBorrowBankConfigure/getGCDiscardInfo");
	$("#disCardForm").submit();
	return false;
}
</script>
...
<body>
	...
	<div class="control-group" >
      ...
      <form:form id="disCardForm" action="${ctx }/channel/jyj/JyjBorrowBankConfigure/getGCDiscardInfo" modelAttribute="LoanFlowQueryParam" method="post">
