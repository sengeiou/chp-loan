...
<title>放款确认</title>
<script src="${context}/js/channel/jyj/jyjurgeGuarante.js" type="text/javascript"></script>
<style type="text/css">
#file {
	border-radius: 8px;
	width:70%;
...
<body>
	<div class="tright pr30 pt5 mb5">
		<button class="btn btn-small" id="btHistory" sid="${UrgeServicesMoneyEx.urgeId}">历史</button>
		<button class="btn btn-small" id="btCheck" sid="${UrgeServicesMoneyEx.urgeId}">提交查账</button>
	</div>
	<h2 class="f14 pl10">综合费用</h2>
	<form id="applyPayLaunchForm" enctype="multipart/form-data" action="${ctx}/channel/jyj/urgeGuaranteeMoney/save" method="post">
	<div class="box2">
		<table class="table1" cellpadding="0" cellspacing="0" border="0"
			...
			<tr>
				...
				<%-- <td><label class="lab">借款类型：</label>${UrgeServicesMoneyEx.classType}</td> --%>
				<td><label class="lab">借款产品：</label>${UrgeServicesMoneyEx.productName}
				<td>
			</tr>

			<tr>
				<td><label class="lab">费用总金额：</label>${UrgeServicesMoneyEx.allFee}</td>
				<td><label class="lab">待收取金额：</label>${UrgeServicesMoneyEx.waitUrgeMoeny}</td>
				<input type="hidden" id="waitUrgeMoney" value="${UrgeServicesMoneyEx.waitUrgeMoeny}"/>
				<td><label class="lab">首次放款时间：</label>
			...
			<tr>
				...
				<td><label class="lab">首次放款金额：</label>
				<fmt:formatNumber value='${UrgeServicesMoneyEx.firstGrantAmount}' pattern="#,#00.00" /></td>
		...
		<div id="qishu_div1" class="box2" style="border-top:0;border-bottom:0">
			<table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
				<tr>
					...
					<td><label class="lab">查账申请日期：</label> 
						<input type="text" class="input_text178" name="urgeServicesCheckApply.urgeApplyDate" value="<fmt:formatDate value="${UrgeServicesMoneyEx.urgeServicesCheckApply.urgeApplyDate }" type="date"/>" readonly>
					</td>
				</tr>
			</table>
		</div>
