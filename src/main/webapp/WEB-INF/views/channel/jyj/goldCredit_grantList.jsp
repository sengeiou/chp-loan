...
$(function(){
	...
	$("#exportExcel").click(function(){
		...
		if($(":input[name='checkBoxItem']:checked").length>0){
			$(":input[name='checkBoxItem']:checked").each(function(){
        		if(selected!="")
        		{
        			selected+=","+$(this).attr("applyId");
        		}else{
        			selected=$(this).attr("applyId");
        		}
        	});
		}		
		var url = ctx+"/channel/jyj/JyjBorrowBankConfigure/exportExcel?idVal="+selected+"&"+queryParam;
		window.location.href=url;
	});
	
	// 导出宝付
	$("#exportExcelBF").click(function(){
		var queryParam = $("#grantForm").serialize();
		var selected = "";
		if($(":input[name='checkBoxItem']:checked").length>0){
			$(":input[name='checkBoxItem']:checked").each(function(){
		...
		window.location.href = ctx+"/channel/jyj/JyjBorrowBankConfigure/exportExcelBF?idVal="+selected+"&"+queryParam;
	});
	...
	$("#handSureGrant").click(function(){
		...
		art.dialog.confirm('确认手动放款么？',function(){
			...
			$.ajax({
				type : "post",
				url : ctx + '/channel/jyj/JyjBorrowBankConfigure/updateGrant',
				data : {'contractCode':contractCode,param},
				dataType : 'text',
				success : function (data){
					dialog.close();
					art.dialog.alert(data);
					window.location=ctx+'/channel/jyj/JyjBorrowBankConfigure/getGCGrantInfo';
				},
				error : function(){
					dialog.close();
		...
		function(){
			art.dialog.tips('手动确认取消');
		});
	});
	
	// 手动确认失败
	$(":button[name='sureGrantFalse']").each(function(){
		$(this).bind('click',function(){
			var contractCode = $(this).attr("contractCode");
			art.dialog.confirm("是否确认失败？",function(){
				$.ajax({
					type:'post',
					data:{'contractCode':contractCode},
					url:ctx+'/channel/jyj/JyjBorrowBankConfigure/sureGrantFail',
					success:function(data){
						art.dialog.alert(data);
						window.location.reload();
					},
					error:function(data){
						art.dialog.alert("处理失败");
					}
				});
			},
			function(){
				art.dialog.tips("手动确认失败取消");
			});
		});
	});
	...
	$("#GCbackBatchSure").click(function(){
		  ...
		  if(backBatchReason==''){
			  ...
			  $.ajax({
					type : "post",
					url : ctx + '/channel/jyj/JyjBorrowBankConfigure/batchBack',
	...
	$("#checkAll").click(function(){
		...
		if ($(this).is(":checked")) {
			$("input[name=checkBoxItem]").each(function() {
				...
				checkedLength += 1;
				totalGrantFailMoney += parseFloat($(this).attr("totalGrantFailMoney"));
			...
			$("#totalGrantFailMoney").text(fmoney(totalGrantFailMoney,2));
...
function page(n, s) {
	...
	if (s)
		$("#pageSize").val(s);
	$("#grantForm").attr("action","${ctx }/channel/jyj/JyjBorrowBankConfigure/getGCGrantInfo");
	$("#grantForm").submit();
	return false;
}
</script>
...
<body>
	<div class="control-group">
        <form:form id="grantForm" action="${ctx }/channel/jyj/JyjBorrowBankConfigure/getGCGrantInfo" modelAttribute="LoanFlowQueryParam" method="post">
          ...
          <table class=" table1" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <tr>
                <td><label class="lab">门店：</label><input type="text"
							class="input_text178" name="storeName" id="storeName"
							readonly="readonly" value="${storeName }" /> <i
				...
				<td><label class="lab">是否电销：</label><form:select class="select180" path="telesalesFlag"><option value="">请选择</option>
    ...
    <p class="mb5">
		...
		<button id="handSureGrant" class="btn btn-small">手动确认放款</button>
    	<button id="GCbatchBackBtn" role="button" class="btn btn-small" data-toggle="modal">批量退回</button>
    	<button id="exportExcelBF" class="btn btn-small">导出宝付</button>
    	&nbsp;&nbsp;<label class="lab1">放款总笔数：</label><label class="red" id="totalNum">0</label>笔
    	&nbsp;&nbsp;<label class="lab1">放款总金额：</label><label class="red" id="totalGrantMoney">0.00</label>元     	
    	&nbsp;&nbsp;<label class="lab1">放款失败总金额：</label><label class="red" id="totalGrantFailAmount">0.00</label>元 </p>
      ...
      <tbody>
       <c:if test="${ workItems!=null && fn:length(workItems)>0}">
          ...
          <c:set var="index" value="${index+1}" />
             <tr <c:if test="${item.receiptResult=='1'}">
                    class='trRed'
               </c:if>>
             <td><input id="checkIndex_${index}" type="checkbox" name="checkBoxItem"
             contractCode="${item.contractCode }" applyId="${item.applyId}" grantAmount='${item.lastGrantAmount}' totalGrantFailMoney='${item.totalGrantFailMoney}' issplit="${item.issplit}" 
             ...
             <td><fmt:formatNumber value='${item.lastGrantAmount}' pattern="#,#00.00"/></td>
             <td>${item.replyMonth}</td>
             <td>金信</td>
             <td>${item.cautionerDepositBank}</td>
             <td>${item.telesalesFlag}</td>
             <td>${item.loanStatusName}</td>
             <td><c:if test="${item.receiptResult=='1'}">放款失败</c:if></td>
             <td>${item.channelName}</td>
             <th>${item.urgentFlag}</th>
             ...
             <td style="position:relative">
				<input type="button" class="btn btn_edit edit" value="操作"/>
				 <div class="alertset" style="display:none">
				 	...
				 	<c:if test="${item.receiptResult!='1' }">
				 	<button class="btn_edit" name="sureGrantFalse" contractCode = "${item.contractCode}">手动确认失败</button>
				 	</c:if>
				 </div>
			 </td>        
            </tr> 
...
<div class="modal fade" id="uploadAuditedModal" tabindex="-1"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<form id="uploadAuditForm" role="form" enctype= "multipart/form-data" method="post" action="${ctx}/channel/jyj/JyjBorrowBankConfigure/importResult">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
