...
<body>
	<div class="control-group">
		<form:form method="post" modelAttribute="params" action="${ctx}/channel/financial/carBusiness/init"
			id="financesForm">
	...
	<p class="mb5">
		&nbsp;&nbsp;<label class="lab1"><span class="red"> 债权合计金额:</span></label><label
			...
			class="red">总笔数:</span></label> <label class="red">${count }</label><label class = "red">&nbsp;笔</label> <input
		...
		<table id="tableList" class="table  table-bordered table-condensed table-hover ">
			...
			<tbody>
				<c:if test="${ financialBusiness!=null && fn:length(financialBusiness.list)>0}">
					<c:forEach items="${financialBusiness.list}" var="item" varStatus="index">
						<tr>
							<td>${index.count}</td>
							<td>${item.loanCode}</td>
