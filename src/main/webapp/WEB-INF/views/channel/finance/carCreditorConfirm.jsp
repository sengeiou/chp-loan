...
<title>车借债权确认列表</title>
<script type="text/javascript" src="${context}/js/common.js"></script>
<script type="text/javascript" src = "${context}/js/channel/finance/creditorCarConfirmList.js">
</script>
<script type="text/javascript">
	...
	function page(n,s){
		if(n) $("#pageNo").val(n);
		if(s) $("#pageSize").val(s);
		$("#confirmForm").attr("action","${ctx }/channel/financial/carConfirm/init");
		$("#confirmForm").submit();
    	return false;
    }
</script>
...
<body>
	<div class="control-group">
		<form:form method="post" modelAttribute="creditorComfirmParam"
			id="confirmForm" action="${ctx }/channel/financial/carConfirm/init">
			...
			<table class="table1" cellpadding="0" cellspacing="0" border="0"
				...
				<tr>
					...
					<form:input type="text" class="input_text178" path="loanCode" style="width:200px;"></form:input></td>
	...
	<p class="mb5">
		...
		&nbsp;&nbsp;<label class="lab1"><span class="red">债权合计金额：</span></label><label id = "amount"
			...
			class="lab1"><span class="red">总笔数：</span></label> <label class="red" id = "count">${count }</label><label
			class="red">笔</label> <input type="hidden" id="num" value="${count }">
	...
	<div class="box5" style="position:relative;width:100%;overflow:hidden;height:60%;margin-bottom: 20px">
		...
		<table id="tableList" class="table  table-bordered table-condensed table-hover " >
			<thead>
				...
				<c:if test="${creditorConfirm!=null && fn:length(creditorConfirm.list)>0}">
					...
					<c:forEach items="${creditorConfirm.list}" var="item">
						...
						<tr>
							...
							<td>${fncjk:getPrdLabelByTypeAndCode('products_type_car_credit',item.classType)}</td>
