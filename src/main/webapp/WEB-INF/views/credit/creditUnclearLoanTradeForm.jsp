...
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
	<div id="unclearLoanTradeTab" >
		<form id="unclearLoanTradeForm">
		
		<table id="unclearLoanTradeTable" cellpadding="0" cellspacing="0" border="0"  width="100%" class="table2">
			<thead>
				<tr>					
					<th>授信机构</th>
					<th>币种</th>
			...
			<c:forEach var="unclearLoan" items="${unclearedImproperLoanList}" varStatus="sta">
				<c:if test="${unclearLoan.businessType eq 2}">
				<tr id="infoTr${sta.index + 1}">
					...
					<td>
						...
						<input  name="businessAmountName" class="input_text70 " type="text" money="1" maxlength="12" 
							...
							onblur="formatMoney('unclearLoanTrade', '${sta.index}', 'businessAmount');" />
					</td>	
					<td>
						...
						<input  name="businessBalanceName" class="input_text70 " type="text" money="1" maxlength="12" 
							...
							onblur="formatMoney('unclearLoanTrade', '${sta.index}', 'businessBalance');" />
					</td>				
					...
					<td><a name="aRemoveInfo" onClick="removeTr('unclearLoanTrade', '${sta.index + 1}');" href="javascript:void(0);" >删除</a></td>
		...
		<table id="unclearLoanTradeTableHide" class="hide">
			<tbody>
				<tr id="infoTr">
					...
					<td>
						<select name="dictGuarantee" class="select78 " id="dictGuarantee">
							<option value="" >请选择</option>
							<c:forEach items="${fns:getDictList('jk_enterprise_guarantee')}" var="item">
								<option value="${item.value}"  >${item.label}</option>
							</c:forEach>
