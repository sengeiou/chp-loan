...
<script src="${context}/js/car/grant/carGrantDeducts.js" type="text/javascript"></script>
<script>
...
function page(n, s) {
	...
	if (s)
		$("#pageSize").val(s);
	$("#deductsForm").attr("action", "${ctx}/car/grant/grantDeducts/deductsInfo?returnUrl=carGrantDeductsList&result=today");
	$("#deductsForm").submit();
	return false;
}

$(document).ready(function(){
	grantDeducts.inintIsAutomatic($("#isAutomatic").val());
...
<body>
	<div class="control-group">
         ...
         <form:form   method="post" modelAttribute="urgeMoneyEx" id="deductsForm" action="${ctx }/car/grant/grantDeducts/deductsInfo?returnUrl=carGrantDeductsList&result=today">
        ...
        <table id = "searchTable" class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <tr>
                ...
                <td name="deductsMoneyValidate" ><label class="lab">划扣金额：</label>
                <form:input id="urgeMoenyStart" type="text" class="input_text70"  path="urgeMoenyStart" ></form:input>-<form:input id="urgeMoenyEnd" type="text" class="input_text70"  path="urgeMoenyEnd" ></form:input>
                </td>
				<td><label class="lab">划扣平台：</label>
				<form:select class="select180" path="dictDealType">
			...
			<tr id="T1" style="display:none">
				 ...
				 <form:select class="select180" path="customerTelesalesFlag"><option value="">请选择</option>
    	...
    	<input type="hidden" id="isAutomatic" value="${isAutomatic}">
		...
		<div>
    	 <sys:columnCtrl pageToken="carGrantDeductsList"></sys:columnCtrl>
    	</div>
        ...
        <tbody>
         ...
         <c:set var="index" value="0"/>
          ...
          <c:set var="index" value="${index+1}" />
             ...
             <td>
             <button class="btn_edit" onclick="showCarLoanHis('${item.loanCode}')">历史</button>
             <button class="btn_edit" id="butss" onclick="showCarLoanInfo('${item.loanCode}')">查看</button>
             </td>
         </tr> 
         </c:forEach>  
         </c:if>
    ...
    <div class="modal-dialog">
       ...
       <div class="modal-body">
       <form id ="exportDeductPlat" method="GET">
       		    <table class="table4" cellpadding="0" cellspacing="0" border="0" width="100%">
             <tr>
                <td><label class="lab"><span style="color: red;">*</span>导出平台：</label>
    ...
    <div class="modal-dialog">
       ...
       <table class="table4" cellpadding="0" cellspacing="0" border="0" width="100%">
             <tr>
				...
				<select id="onlineDeducts">
				    		<option value="">请选择</option>
						<c:forEach items="${fns:getDictLists('0,1,2,3', 'jk_deduct_plat')}" var="item">
								<option value="${item.value}">${item.label}</option>
						</c:forEach>
				 </select>
				</td>
    ...
    <div class="modal-dialog">
       ...
       <table class="table4" cellpadding="0" cellspacing="0" border="0" width="100%">
       	             ...
       	             <tr>
                <td><label class="lab">划扣平台：</label>
				<select id="manualDeducts" class="select180 required " >
				    		<option value="">请选择</option>
						<c:forEach items="${fns:getDictLists('1,2,3', 'jk_deduct_plat')}" var="item">
								<option value="${item.value}">${item.label}</option>
						</c:forEach>
