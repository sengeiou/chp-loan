...
import com.creditharmony.loan.channel.jyj.dao.JyjUrgeServicesMoneyAlreadyDao;
 ...
 */
@Service("JyjGrantDeductsAlreadyService")
public class JyjGrantDeductsAlreadyService extends CoreManager<JyjUrgeServicesMoneyAlreadyDao, UrgeServicesMoneyEx>{
	@Autowired
	...
	public List<DeductReq> selectSendList(UrgeServicesMoneyEx urgeMoney){
		...
		for(DeductReq deductReq:list){
			...
			// 设置划扣备注
			deductReq.setRemarks(deductReq.getBusinessId()+"__综合费用");
			//  系统处理ID，设置为催收处理
