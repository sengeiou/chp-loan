...
import com.creditharmony.loan.channel.goldcredit.dao.SummaryDao;
...
@Service
public class SummaryService extends CoreManager<SummaryDao, Summary>{
	@Autowired
	...
	public List<Object> findGCCeilingList(Page<Summary> page, Summary params) {
		PageBounds pageBounds = new PageBounds(page.getPageNo(),page.getPageSize());
		String [] storeOrgIds = {};
		if (StringUtils.isNotEmpty(params.getStoreOrgId())){
			storeOrgIds = params.getStoreOrgId().split(",");
		}
		if (ObjectHelper.isNotEmpty(params.getReferralsDate())){
			params.setReferralsDates(format.format(params.getReferralsDate()));
		}
		pageBounds.setFilterOrderBy(BooleanType.FALSE);
		Map<String ,Object> conditions = Maps.newHashMap();
		conditions.put("params", params);
		conditions.put("storeOrgIds", storeOrgIds);
		PageList<Summary> pageList = (PageList<Summary>) dao.getCeilingList(pageBounds, conditions);
	...
	/**
	 *  统计
	    * @Description: TODO(统计导出金额)  
	    * @date 2017年6月7日下午4:02:14 
	    * @author wengsi
	...
	public Map<String, Object> getCeilingSumExcel(Map<String, Object> conditions) {
		return  dao.getCeilingSumExcel(conditions);
	}
	/**
	 * 导出
	 * 2016年2月24日 
	 ...
	 */
	@Transactional(readOnly = true,value = "loanTransactionManager")
	public List<Summary> exportCeiling(String cid,Summary params){
		Map<String ,Object> conditions = Maps.newHashMap();
		if (StringUtils.isNotEmpty(params.getStoreOrgId())){
		...
		conditions.put("params", params);
		List<Summary> pageList = dao.getCeilingList(conditions);
		return pageList;
	}
	/**
	 * 获取用户申请信贷信息
	 ...
	 * @return
	 */
	public List<Map<String, String>> queryCellingNum(Summary cellingNum) {
		Map<String ,Object> conditions = Maps.newHashMap();
	 ...
	 * 
	    * @Description: 查询导出次数
	    * @date 2017年6月8日上午10:10:56 
	    * @author wengsi
	    * @param @param parm
	    * @param @return    参数  
	    * @return SummaryCount    返回类型
	 */
	public SummaryCount querySummaryExport(SummaryCount parm) {
		return dao.querySummaryExport(parm);
	}

	/**
	 * 
	    * @Description:增加导出次数
	    * @date 2017年6月8日上午10:11:10 
	    * @author wengsi
	    * @param @param parm    参数  
	    * @return void    返回类型
	 */
	public void saveSummaryExport(SummaryCount parm) {
		dao.saveSummaryExport(parm);
	}

	/**
	 * 
     * @Description:修改导出次数
     * @date 2017年6月8日上午10:11:26 
     * @author wengsi
     * @param @param parm    参数  
     * @return void    返回类型
	 */
	public void updateSummaryExport(SummaryCount parm) {
		dao.updateSummaryExport(parm);
	}
