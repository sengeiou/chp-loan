 ...
 */
@Service
public class CarExtendWorkFlowCommonService {
	@Autowired
	...
	protected Map<String, String> carContractHandle(CarCheckRateBusinessView flowView, String stepName) {
		...
		String loanStatus = flowView.getApplyStatusCode();
		if (CarLoanResponses.TO_SIGN.getCode().equals(checkResult)) {
			Double extFee = flowView.getExtensionFee(); // 展期费用
			if (extFee != null) {
				// 工具类封装合同信息，用于更改表中展期费用的值
				CarContract carContract = CarLoanPreUpdateUtil.UpdateCarContract2(loanCode, new BigDecimal(extFee));
		...
		String stepNode = CarLoanSteps.parseByName(stepName).getCode(); // 审批步骤
