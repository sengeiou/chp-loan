 ...
 */
@Service
public class ChangJieCertifSignService {
	 ...
	 * 日志
	 */
	private static final Logger log = LoggerFactory.getLogger(ChangJieCertifSignService.class);
	...
	public PayCertificationSingleInBean changJieCertifSignService(PayCertificationSingleInBean cjDetail){
		...
		log.debug("畅捷认证接口===开始,流水号为：" + cjDetail.getTrSerialNo());
		BaseOutInfo outInfo = (BaseOutInfo) service.callService(cjDetail);
		log.debug("畅捷认证接口===结束,流水号为：" + cjDetail.getTrSerialNo());
		...
		if ("0000".equals(outInfo.getRetCode())) {
            ...
            loanBank.setCjSignFailure(retMsg);
