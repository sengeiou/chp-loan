 ...
 */
public class ExportCenterListDeductHelper {
   ...
   try {
	...
	public static void exportCenterTodo(PaybackApply queryMap,
		...
		try {
			...
			MyBatisSql batisSql = MyBatisSqlUtil
					.getMyBatisSql(
							"com.creditharmony.loan.borrow.payback.dao.CentralizedListDao.centerDeductionAgencyList",
							queryMap, sqlSessionFactory);
			PreparedStatement ps = connection.prepareStatement(batisSql
					.toString());
			ResultSet resultSet = ps.executeQuery();
	...
	private static void ToDoExcelCell(ResultSet resultSet, Sheet dataSheet)
		...
		while (resultSet.next()) {
			...
			orgName = resultSet.getString("orgName");//门店名称
			applyBankName = resultSet.getString("applyBankName");//开户行名称
			contractMonths = resultSet.getString("contractMonths");//批借期数
			contractReplayDay = resultSet.getString("contractReplayDay");//首期还款期
			applyReallyAmount = resultSet.getString("applyReallyAmount");//实还金额
			paybackMonthAmount = resultSet.getString("paybackMonthAmount");//期供
			notPaybackMonthAmount = resultSet.getString("notPaybackMonthAmount");//当期未还期供
			alsoPaybackMonthAmount = resultSet.getString("alsoPaybackMonthAmount");//当期已还期供
			applyDeductAmount = resultSet.getString("applyAmount");//划扣金额
			
			huankType = "集中划扣";
			
			dictLoanStatus = resultSet.getString("dictLoanStatus");//借款状态
			billDay = resultSet.getString("billDay");//账单日
			paybackDay = resultSet.getString("monthPayDay");//期供还款日
			paybackMaxOverduedays = resultSet.getString("paybackMaxOverduedays");//往期是否逾期
			loanMark = resultSet.getString("loanMark");//渠道
			loanModel = resultSet.getString("model");//渠道
			
		    
			paybackBuleAmount = resultSet.getString("paybackBuleAmount");//蓝补金额
			bankAccount = resultSet.getString("bankAccount");//卡号
			...
			huankTypeCell = dataRow.createCell(11);
...
//			dictLoanStatusCell.setCellStyle(contentStyle);
			...
			modifyTimeCell = dataRow.createCell(12);
			modifyTimeCell.setCellValue(dictLoanStatus);
			modifyTimeCell.setCellStyle(contentStyle);
			
			billDayCell = dataRow.createCell(13);
			billDayCell.setCellValue(billDay);
			billDayCell.setCellStyle(contentStyle);
			
			paybackDayCell = dataRow.createCell(14);
			paybackDayCell.setCellValue(paybackDay);
			paybackDayCell.setCellStyle(contentStyle);
			
			whetherOverdueCell = dataRow.createCell(15);
			if (StringUtils.isNumeric(paybackMaxOverduedays)
					&& Integer.parseInt(paybackMaxOverduedays) > 0) {
				whetherOverdueCell.setCellValue("是");
			} else {
				whetherOverdueCell.setCellValue("否");
			}
			whetherOverdueCell.setCellStyle(contentStyle);
			
			loanMarkCell = dataRow.createCell(16);
			loanMarkCell.setCellValue(loanMark);
			loanMarkCell.setCellStyle(contentStyle);
			
			loanModelCell = dataRow.createCell(17);
			if(LoanModel.TG.getCode().equals(loanModel))
			{
				loanModel="TG";
			}
			else
			...
			loanModelCell.setCellValue(loanModel);
			loanModelCell.setCellStyle(contentStyle);
			...
			bankAccountCell.setCellStyle(contentStyle);
			 ...
			 huaDate =  resultSet.getDate("modify_time");
			 String hy = "";
			 if(huaDate != null){
			 ...
			 if(CounterofferResult.PREPAYMENT.getCode().equals(huipan))
			 {
				 huipan ="失败";
			 }
			 if(CounterofferResult.PROCESS.getCode().equals(huipan)||CounterofferResult.PROCESS.getCode().equals(huipan))
			 {
				 huipan ="处理中";
			 }
			 if(CounterofferResult.PAYMENT_SUCCEED.getCode().equals(huipan))
			 {
				 huipan ="成功";
			 }
			if (CounterofferResult.PAYMENT_FAILED.getCode().equals(huipan)
					|| CounterofferResult.PAYMENT_CONTINUE.getCode().equals(
							huipan)
					|| CounterofferResult.PROCESS_TIMEOUT.getCode().equals(
							huipan)) {
				huipan = "失败";
			}
			 ...
			 huaPing =  resultSet.getString("dictDealTypeLabel");
	...
	private static void toDoHeader(Sheet dataSheet) {
		...
		Cell memoHeader = headerRow.createCell(11);
		memoHeader.setCellStyle(dateStyle);
...
//		backiIdHeader.setCellValue("还款状态");
		...
		Cell applyLendDayHeader = headerRow.createCell(12);
		applyLendDayHeader.setCellStyle(dateStyle);
		applyLendDayHeader.setCellValue("借款状态");
		
		Cell applyLendMoneyHeader = headerRow.createCell(13);
		applyLendMoneyHeader.setCellStyle(dateStyle);
		applyLendMoneyHeader.setCellValue("账单日");
		
		Cell applyPayHeader = headerRow.createCell(14);
		applyPayHeader.setCellStyle(dateStyle);
		applyPayHeader.setCellValue("还款日");
		
		Cell yuqiHeader = headerRow.createCell(15);
		yuqiHeader.setCellStyle(dateStyle);
		yuqiHeader.setCellValue("往期是否逾期");
		
		Cell biaoshiHeader = headerRow.createCell(16);
		biaoshiHeader.setCellStyle(dateStyle);
		biaoshiHeader.setCellValue("渠道");
		
		Cell modelHeader = headerRow.createCell(17);
		modelHeader.setCellStyle(dateStyle);
		modelHeader.setCellValue("模式");
