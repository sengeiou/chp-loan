 ...
 */
@Service("grantDeductsService")
public class GrantDeductsService extends CoreManager<UrgeServicesMoneyDao, UrgeServicesMoneyEx>{
	@Autowired
	...
	public List<DeductReq> selectSendList(UrgeServicesMoneyEx urgeMoney){
		...
		for(DeductReq deductReq:list){
			...
			// 设置划扣备注
			deductReq.setRemarks(deductReq.getBusinessId()+"_催收服务费");
			//  系统处理ID，设置为催收处理
	...
	public Page<UrgeServicesMoneyEx> selectDeductsJYJSuccess(
			Page<UrgeServicesMoneyEx> page, UrgeServicesMoneyEx urgeMoneyEx) {
		PageBounds pageBounds = new PageBounds(page.getPageNo(),page.getPageSize());
		PageList<UrgeServicesMoneyEx> pageList = (PageList<UrgeServicesMoneyEx>)dao.selectDeductsJYJSuccess(pageBounds, urgeMoneyEx);
		PageUtil.convertPage(pageList, page);
		return page;
	}
