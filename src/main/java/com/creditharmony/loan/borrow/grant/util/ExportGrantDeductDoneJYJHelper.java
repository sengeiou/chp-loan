...
public class ExportGrantDeductDoneJYJHelper {

	private static Logger logger = LoggerFactory.getLogger(ExportGrantDeductDoneJYJHelper.class);
	 ...
	 public static void exportData(UrgeServicesMoneyEx urgeEx,String[] header,
			...
			try {
				String fileName = DateUtils.getDate("yyyyMMdd")+"简易借已划扣列表";
				...
				MyBatisSql batisSql = MyBatisSqlUtil
						.getMyBatisSql(
								"com.creditharmony.loan.borrow.grant.dao.UrgeServicesMoneyDao.selectDeductsJYJSuccess",
		...
		private static void assembleExcelCell(SXSSFWorkbook wb,ResultSet resultSet, Sheet dataSheet,int MAXCOLUMN)
			...
			while (resultSet.next()) {
				...
				Cell firstGrantAmountCell = dataRow.createCell(5);
				firstGrantAmountCell.setCellStyle(style);
				firstGrantAmountCell.setCellValue(resultSet.getString("firstGrantAmount"));
				Cell allFeeCell = dataRow.createCell(6);
				allFeeCell.setCellStyle(style);
				allFeeCell.setCellValue(resultSet.getString("allFee"));
				Cell feeCountCell = dataRow.createCell(7);
				feeCountCell.setCellStyle(style);
				feeCountCell.setCellValue(resultSet.getString("feeCount"));
				Cell feePetition = dataRow.createCell(8);
				feePetition.setCellStyle(style);
				feePetition.setCellValue(resultSet.getString("feePetition"));
				Cell feeExpeditedCell = dataRow.createCell(9);
				feeExpeditedCell.setCellStyle(style);
				feeExpeditedCell.setCellValue(resultSet.getString("feeExpedited"));
				
				
				
				Cell loanCustomerServiceCell = dataRow.createCell(10);
				loanCustomerServiceCell.setCellStyle(style);
				loanCustomerServiceCell.setCellValue(format(resultSet.getString("feeUrgedService"), "##0.00"));
				Cell contractCodeCell = dataRow.createCell(11);
				contractCodeCell.setCellStyle(style);
				contractCodeCell.setCellValue(format(resultSet.getString("waitUrgeMoeny"), "##0.00"));
				
				Cell auditAmountCell = dataRow.createCell(12);
				auditAmountCell.setCellStyle(style);
				auditAmountCell.setCellValue(format(resultSet.getString("splitAmount"),"##0.00"));
				Cell feePetitionCell = dataRow.createCell(13);
				...
				dataSheet.setColumnWidth(22,10000);
				Cell checkEmpIdCell = dataRow.createCell(22);
