...
public class ExcelExportCeilingHelper {

	private static Logger logger = LoggerFactory.getLogger(ExcelExportCeilingHelper.class);
	...
	public static void exportData(Map<String, Object> queryMap,
		...
		try {
			String fileName = FileExtension.TRUSTEESHIP_CHANNEL + System.currentTimeMillis() ;
			...
			MyBatisSql batisSql = MyBatisSqlUtil
					.getMyBatisSql(
							"com.creditharmony.loan.borrow.trusteeship.dao.GoldAccountCeilingDao.getCeilingList",
	...
	private static void assembleExcelCell(SXSSFWorkbook wb,ResultSet resultSet, Sheet dataSheet,int MAXCOLUMN)
		...
		while (resultSet.next()) {
			...
			Cell loanStatusCell = dataRow.createCell(11);
			loanStatusCell.setCellStyle(style);
			loanStatusCell.setCellValue(resultSet.getString("dict_loan_status"));
			Cell grantDateCell = dataRow.createCell(12);
			grantDateCell.setCellStyle(style);
			grantDateCell.setCellValue(resultSet.getString("lending_time"));
			Cell dataStatusCell = dataRow.createCell(13);
			dataStatusCell.setCellStyle(style);
			dataStatusCell.setCellValue(resultSet.getString("state"));
			Cell logoCell = dataRow.createCell(14);
			logoCell.setCellStyle(style);
			logoCell.setCellValue(resultSet.getString("loan_flag"));
			row = row + 1;
		}
	...
	private static void wrapperHeader(SXSSFWorkbook wb,Sheet dataSheet,String fileName,int MAXCOLUMN) {
		...
		Cell loanStatusCellHeader = headerRow.createCell(11);
		loanStatusCellHeader.setCellStyle(style);
		loanStatusCellHeader.setCellValue("借款状态");
		Cell grantDateCellHeader = headerRow.createCell(12);
		grantDateCellHeader.setCellStyle(style);
		grantDateCellHeader.setCellValue("放款日期");
		Cell dataStatusCellHeader = headerRow.createCell(13);
		dataStatusCellHeader.setCellStyle(style);
		dataStatusCellHeader.setCellValue("数据状态");
		Cell logoCellHeader = headerRow.createCell(14);
		logoCellHeader.setCellStyle(style);
		logoCellHeader.setCellValue("标识");
	}
	/**
