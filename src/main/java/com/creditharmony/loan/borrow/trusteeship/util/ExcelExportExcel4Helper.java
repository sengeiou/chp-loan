...
public class ExcelExportExcel4Helper {

	private static Logger logger = LoggerFactory.getLogger(ExcelExportExcel4Helper.class);
	...
	public static void exportData(Map<String, Object> queryMap,
		...
		try {
			String fileName = FileExtension.TRUSTEESHIP_PENDING_LOAN4 + System.currentTimeMillis() ;
	...
	private static void assembleExcelCell(SXSSFWorkbook wb,ResultSet resultSet, Sheet dataSheet,int MAXCOLUMN)
		...
		while (resultSet.next()) {
			...
			tradeMoneyCell.setCellValue(grantMoney.subtract(urgedServiceFeeMoney).toString());
