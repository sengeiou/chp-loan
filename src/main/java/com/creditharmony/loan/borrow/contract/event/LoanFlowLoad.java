 ...
 * @version 1.0
 */
@Service("load_hj_loanFlow")
public class LoanFlowLoad extends BaseService implements BusinessLoadCallBack {
	...
	public BaseBusinessView load(String applyId, String stepName) {
		...
		if (LoanCustomerEx.INFORMATION_UPLOAD.equals(stepName)) {
			...
			LoanCustomer loanCustomer = contractCustInfoServcie.selectByApplyId(applyId);
			if (!ObjectHelper.isEmpty(loanCustomer)) {
				String loanCode = loanCustomer.getLoanCode();
				List<LoanCoborrower> coborrowers = loanService.selectByLoanCode(loanCode);
				uploadView.setLoanCustomer(loanCustomer);
				uploadView.setCoborrowers(coborrowers);
				/** 查询借款信息 start */
				Map<String, Object> param = new HashMap<String, Object>();
				param.put("loanCode", loanCode);
				LoanInfo loanInfo = loanInfoDao.selectByLoanCode(param);
				uploadView.setLoanInfo(loanInfo);
				/** 查询借款信息 end */
			}
			String imageUrl = imageService.getImageUrl(stepName, loanCustomer.getLoanCode());
			uploadView.setImageUrl(imageUrl);
			uploadView.setApplyId(applyId);
			...
			uploadView.setIsStoreAssistant(isStoreAssistant);
			// 缓存码表取值
			LoanCustomer lc = uploadView.getLoanCustomer();
			...
			view = uploadView;
			// 门店复核//复议退回
		} else if (LoanCustomerEx.STORE_REVERIFY.equals(stepName)
				|| LoanCustomerEx.RECONSIDER_BACK_STORE.equals(stepName)) {
			...
			if ("1".equals(LoanInfoOldOrNewFlag)) {
				...
				// 设置preResponse
				String dictLoanStatus = areaService.getStatusByApplyId(applyId);
				if (LoanApplyStatus.APPLY_ENGINE_BACK.getCode().equals(dictLoanStatus)
						|| LoanApplyStatus.STORE_REVERIFY.getCode().equals(dictLoanStatus)) {
					launchView.setPreResponse("TO_RULEENGIN");
				} else if (LoanApplyStatus.BACK_STORE.getCode().equals(dictLoanStatus)) {
					launchView.setPreResponse("STORE_TO_CHECK");
				}
				//
				User user = UserUtils.getUser();
				List<String> roleIds = user.getRoleIdList();
				...
				List<CityInfo> provinceList = cityInfoDao.findByParams(params);
				launchView.setProvinceList(provinceList);
				ApplyInfoFlagEx applyInfo = dataEntryService.getAllInfo(applyId);
				BeanUtils.copyProperties(applyInfo, launchView);
				String dictLoanStatus = areaService.getStatusByApplyId(applyId);
				if (LoanApplyStatus.APPLY_ENGINE_BACK.getCode().equals(dictLoanStatus)
						|| LoanApplyStatus.STORE_REVERIFY.getCode().equals(dictLoanStatus)) {
				...
				if (!ObjectHelper.isEmpty(launchView.getLoanInfo())) {
					...
					String productCode = launchView.getLoanInfo().getProductType();
					if (LoanProductCode.PRO_XYJ.equals(productCode)) {
						isBorrow = "1";
					}
				...
				Iterator<LoanPrdMngEntity> it = productList1.iterator();
				while (it.hasNext()) {
					LoanPrdMngEntity lpme = it.next();
					if ("1".equals(isBorrow)) {
						if (!LoanProductCode.PRO_XYJ.equals(lpme.getProductCode())) {
							it.remove();
						}
					} else {
						if (LoanProductCode.PRO_XYJ.equals(lpme.getProductCode())) {
				...
				if (StringUtils.isNotEmpty(launchView.getLoanInfo().getLoanManagerCode())) {
					launchView.getLoanInfo()
							.setLoanManagerName(areaService.getUserName(launchView.getLoanInfo().getLoanManagerCode()));
				}
				String imageUrl = imageService.getImageUrl(stepName, launchView.getLoanCustomer().getLoanCode());
				launchView.setImageUrl(imageUrl);
				// 缓存码表取值
				...
				lc.setCustomerSexLabel(dictCache.getDictLabel("jk_sex", lc.getCustomerSex()));
				lc.setDictCertTypeLabel(dictCache.getDictLabel("com_certificate_type", lc.getDictCertType()));
				lc.setDictMarryLabel(dictCache.getDictLabel("jk_marriage", lc.getDictMarry()));
				lc.setDictEducationLabel(dictCache.getDictLabel("jk_degree", lc.getDictEducation()));
				// 申请信息
				LoanInfo loanInfo = launchView.getLoanInfo();
				loanInfo.setDictLoanUserLabel(dictCache.getDictLabel("jk_loan_use", loanInfo.getDictLoanUse()));
				loanInfo.setLoanUrgentFlagLabel(dictCache.getDictLabel("jk_urgent_flag", loanInfo.getLoanUrgentFlag()));
				// 信用资料
				List<LoanCreditInfo> list = launchView.getLoanCreditInfoList();
				for (LoanCreditInfo loanCreditInfo : list) {
					loanCreditInfo.setDictMortgageTypeLabel(
							dictCache.getDictLabel("jk_pledge_flag", loanCreditInfo.getDictMortgageType()));
				}
				// 职业信息/公司资料
				LoanCompany loanCompany = launchView.getCustomerLoanCompany();
				loanCompany.setCompPostLabel(dictCache.getDictLabel("jk_job_type", loanCompany.getCompPost()));
				loanCompany.setDictCompTypeLabel(dictCache.getDictLabel("jk_unit_type", loanCompany.getDictCompType()));
				// 房产资料
				List<LoanHouse> hostList = launchView.getCustomerLoanHouseList();
				for (LoanHouse loanHouse : hostList) {
					loanHouse.setHouseBuywayLabel(
							dictCache.getDictLabel("jk_house_buywayg", loanHouse.getHouseBuyway()));
					loanHouse.setHousePledgeFlagLabel(
							dictCache.getDictLabel("jk_pledge_flag", loanHouse.getHousePledgeFlag()));
				}
				// 联系人资料
				List<Contact> contactList = launchView.getCustomerContactList();
				for (Contact contact : contactList) {
					contact.setRelationTypeLabel(dictCache.getDictLabel("jk_relation_type", contact.getRelationType()));
					if ("0".equals(contact.getRelationType())) {
						contact.setContactRelationLabel(
								dictCache.getDictLabel("jk_loan_family_relation", contact.getContactRelation()));
					}
					if ("1".equals(contact.getRelationType())) {
						contact.setContactRelationLabel(
								dictCache.getDictLabel("jk_loan_workmate_relation", contact.getContactRelation()));
					}
					if ("2".equals(contact.getRelationType())) {
						contact.setContactRelationLabel(
								dictCache.getDictLabel("jk_loan_other_relation", contact.getContactRelation()));
					}
				}
				// 银行卡资料
				LoanBank loanBank = launchView.getLoanBank();
				loanBank.setBankNameLabel(dictCache.getDictLabel("jk_open_bank", loanBank.getBankName()));
				loanBank.setBankSigningPlatformName(
						dictCache.getDictLabel("jk_deduct_plat", loanBank.getBankSigningPlatform()));
				launchView.setConsultTime(LoanConsultDateUtils.findTimeByApplyId(applyId));
				view = launchView;
			}

		} else if (ContractConstant.RATE_AUDIT.equals(stepName)) {
			ContractBusiView rateAuditView = new ContractBusiView();
			// ContractBusiView storeInfo =
			// contractService.findStoreInfo(applyId);
			CustInfo custInfo = custInfoService.findCustInfo(applyId);
			String loanCode = null;
			CustInfo auditInfo = custInfoService.findAuditInfo(applyId);
			// 查询外访距离，外访标志
			Map<String, String> outSide = custInfoService.findOutSide(custInfo.getLoanCode());
			if (outSide != null) {
				rateAuditView.setOutside_flag(outSide.get("outside_flag"));
				if (!"1".equals(outSide.get("outside_flag")) || null == outSide.get("item_distance")
						|| "".equals(outSide.get("item_distance"))) {
			...
			if (ObjectHelper.isEmpty(contract)) {
				...
				/*
				 * //根据contract.getLoanCode()去t_jk_audit_rating表查询风险等级
				 * if(!list.isEmpty()){
				 * if(list.get(0)!=null&&!"".equals(list.get(0))){
				 * if(list.get(0).get("reconsiderrisklevel")!=null&&!"".equals(
				 * list.get(0).get("reconsiderrisklevel"))){ riskLevel =
				 * list.get(0).get("reconsiderrisklevel"); }else{ riskLevel =
				 * list.get(0).get("verifyrisklevel"); } } }
				 */
			}
			...
			if (ObjectHelper.isNotEmpty(list) && !ObjectHelper.isEmpty(list.get(0))) {
				if (list.get(0).get("reconsiderrisklevel") != null
						&& !"".equals(list.get(0).get("reconsiderrisklevel"))) {
			...
			if (StringUtils.isNotEmpty(loanCode)) {
				...
				if (!ObjectHelper.isEmpty(outSideInfos)) {
					...
					if (!ObjectHelper.isEmpty(outSideInfo) && !ObjectHelper.isEmpty(outSideInfo.getItemDistance())
							&& outSideInfo.getItemDistance().doubleValue() != 0) {
			...
			FeeInfoEx feeInfoEx = ReckonFeeNew.FeeFormat(rateAuditView.getCtrFee(), rateAuditView.getContract(),
			...
			if (StringUtils.isNotEmpty(rateAuditView.getProductType())) {
				...
				// rateAuditView.getCtrFee().setFeeAllRaio(new
				// BigDecimal(product.getInterestRate()));
			}
			// 设置费率
			...
			if (contract.getAuditAmount().compareTo(new BigDecimal(300000)) > 0) {
				String largeAmountImgUrl = imageService.getImageUrl(FlowStep.LAGE_AMOUNT_VIEW.getName(),
						custInfo.getLoanCode());
			...
			if (DateUtils.dateAfter(consultDate, onLineDate)) {// 根据咨询时间来判断
				...
				if(con.getProductType().equals(ProductUtil.PRODUCT_JYJ.getCode()) || con.getProductType().equals(ProductUtil.PRODUCT_NXD.getCode())){	//todo
				...
				try {
					...
					rateAuditView.getFeeInfo()
							.setFeePaymentAmount(rateAuditView.getCtrFee().getFeePaymentAmount() + "");
					...
					rateAuditView.getFeeInfo()
							.setMonthMidFeeService(rateAuditView.getCtrFee().getMonthMidFeeService() + "");
					rateAuditView.getFeeInfo().setContractMonthRepayAmount(
							rateAuditView.getContract().getContractMonthRepayAmount() + "");
					...
					rateAuditView.getFeeInfo()
							.setMonthPayTotalAmount(rateAuditView.getContract().getMonthPayTotalAmount() + "");
					rateAuditView.getFeeInfo().setFeeCount(rateAuditView.getCtrFee().getFeeCount() + "");
					rateAuditView.getFeeInfo()
							.setContractExpectAmount(rateAuditView.getContract().getContractExpectAmount() + "");
					if (loanInfoObj.getIssplit().equals(ContractConstant.ISSPLIT_1)) {
						// zcj
					...
					BigDecimal feeExpedited = ReckonFeeNew.urgeFee(isUrge, rateAuditView.getContract(),
							rateAuditView.getCtrFee());
					rateAuditView.getFeeInfo().setFeeExpedited(feeExpedited + "");
					rateAuditView.getCtrFee().setFeePaymentAmount(
							rateAuditView.getCtrFee().getFeePaymentAmount().subtract(feeExpedited));
					if (loanInfoObj.getIssplit().equals(ContractConstant.ISSPLIT_1)) {
						// zcj
					...
					// ReflectHandle.copy(rateAuditView.getContract(),
					// rateAuditView.getFeeInfo());
					// ReflectHandle.copy(rateAuditView.getCtrFee(),
					// rateAuditView.getFeeInfo());
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			String back_time = PropertyUtil.getStrValue("application.properties", "back_time", "");
			long backTime = new Date().getTime();
			if (back_time != null && !back_time.equals("")) {
				backTime = Long.parseLong(back_time);

			}
			String propose_time = PropertyUtil.getStrValue(
     				"application.properties", "propose_time", "");
     		long proposeTime = new Date().getTime();
     		if(propose_time!=null && !propose_time.equals("")){
     			proposeTime=Long.parseLong(propose_time);
     			
     		}
			ContractBusiView confirmSign = new ContractBusiView();
			confirmSign.setApplyId(applyId);
			confirmSign.setReconsiderFlag("0");
			CustInfo custInfo = custInfoService.findCustInfo(applyId);
           ...
           confirmSign.setContractTemps(contractTemps);
			if (loanInfo != null) {
				  if(loanInfo.getCustomerIntoTime().getTime()>proposeTime){
	        		   confirmSign.setProposeFlag("0");
	          	  }
			...
			CustInfo auditInfo = custInfoService.findAuditInfo(applyId);
			Contract contract = contractService.findByApplyId(applyId);
			// List<LoanCoborrower> coborrowers=
			// loanService.selectByLoanCode(custInfo.getLoanCode());
			List<LoanCoborrower> coborrowers = null;
			...
			if (coborrowers != null) {
				for (LoanCoborrower loanCoborrower : coborrowers) {
					loanCoborrower.setDictCertTypeName(
							dict.getDictLabel("com_certificate_type", loanCoborrower.getDictCertType()));
			...
			try {
				...
				List<LoanBank> loanBanks = loanBankService.selectByLoanCode(custInfo.getLoanCode(),
						ContractConstant.TOP_FLAG);
			...
			custServiceSign.setReconsiderFlag("0");
			...
			if (!ObjectHelper.isEmpty(contract)) {
				List<LoanBank> loanBanks = loanBankService.selectByLoanCode(contract.getLoanCode(),
						ContractConstant.TOP_FLAG);
				if (!ObjectHelper.isEmpty(loanBanks)) {
            ...
            String back_time = PropertyUtil.getStrValue(
     		...
     		if(back_time!=null && !back_time.equals("")){
     		 ...
     		 if(loanInfo!=null){
            	  if(loanInfo.getCustomerIntoTime().getTime()>backTime){
            		  custServiceSign.setProposeFlag("0");
            	  }else{
            		  custServiceSign.setProposeFlag("1");
            	  }
               }
     		custServiceSign.setLoanInfo(loanInfo);
			custServiceSign.setIsStoreAssistant(isStoreAssistant);
                   ...
                   if(ContractConstant.ISSPLIT_1.equals(loanInfo.getIssplit())){
                      ...
                      // paramMap.put("isreceive", YESNO.YES.getCode());
                       ...
                       if(zcjFeeTemp!=null){
                    	   ...
                    	   contractFee.setFeePetition(BigDecimalTools.add(contractFee.getFeePetition(), zcjFeeTemp.getFeePetitionTemp()));
                       ...
                       if(jinXinFeeTemp!=null){
                    	   ...
                    	   contractFee.setFeePetition(BigDecimalTools.add(contractFee.getFeePetition(), jinXinFeeTemp.getFeePetitionTemp()));
			...
			if(contract.getChannelFlag().equals(ChannelFlag.ZCJ.getCode())){
				if(!contract.getContractVersion().equals(ContractVer.VER_ONE_ONE_ZCJ.getCode()) && !contract.getContractVersion().equals(ContractVer.VER_ONE_ZERO_ZCJ.getCode())){
					...
					Payback payback=paybackDao.selectpayBack(map);
					contractAudit.setBillDay(payback.getPaybackDay());
				}
			}
			...
			String imageUrl = imageService.getImageUrl(FlowStep.CONTRACT_AUDIT_CONTRACT.getName(),
					custInfo.getLoanCode());
			if (contract.getAuditAmount().compareTo(new BigDecimal(300000)) > 0) {
				String largeAmountImgUrl = imageService.getImageUrl(FlowStep.LAGE_AMOUNT_VIEW.getName(),
						custInfo.getLoanCode());
		...
		if (!ObjectHelper.isEmpty(obj)) {
			if (obj instanceof ContractBusiView) {
				...
				if (!ObjectHelper.isEmpty(contractView)) {
					contractView.setMainCertTypeName(
							dict.getDictLabel("com_certificate_type", contractView.getMainCertType()));
					...
					if (!ObjectHelper.isEmpty(loanBank)) {
						loanBank.setBankSigningPlatformName(
								dict.getDictLabel("jk_deduct_plat", loanBank.getBankSigningPlatform()));
						loanBank.setBankNameLabel(dict.getDictLabel("jk_open_bank", loanBank.getBankName()));
						contractView.setLoanBank(loanBank);
					}
					if (!ObjectHelper.isEmpty(contract)) {
						contract.setDictRepayMethodName(
								dict.getDictLabel("jk_repay_interest_way", contract.getDictRepayMethod()));
				...
				if (!ObjectHelper.isEmpty(customerView)) {
					customerView.setDictCertTypeName(
							dict.getDictLabel("com_certificate_type", customerView.getDictCertType()));
